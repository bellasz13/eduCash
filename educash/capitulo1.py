...
É o processo de adquirir conhecimento e autoconfiança para gerenciar eficientemente seus recursos financeiros, compreendendo como tomar decisões financeiras, evitar endividamento e construir um futuro financeiro sustentável. 

Investir tempo e esforço na educação financeira agrega conhecimento e proporciona vários benefícios que impactam positivamente a jornada de cada um. 
Dentre esses benefícios, destacam-se:
...
Em resumo, os juros compostos são uma força poderosa que pode transformar seus investimentos em um patrimônio significativo. Ao entender como eles funcionam e como se aplicam à previdência privada, você estará dando um passo importante para garantir um futuro mais tranquilo e próspero.
    ...
    def subcapitulo1_desafio():
        criar_subcapitulo(
            "Desafio",
            """
Parabéns por ter chegado até  aqui futuro invetidor!
    
Agora está na hora de colocar em prática tudo que você aprendeu no Capítulo 1 - O que é educação financeira e por que ela é importante?

Crie um orçamento detalhado para o próximo mês, estabeleça metas  financeiras e monitore seus hábitos de consumo.

A pergunta que fica agora é: ao final do seu orçamento, você se considera uma pessoa com bons hábitos financeiros?
            """
        )
